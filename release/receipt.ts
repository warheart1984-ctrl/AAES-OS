...
export function createReleaseReceipt(manifest, checksums, extras = {}) {
  ...
  const baseReceipt = {
    ...
    testEvidence: createEvidenceSection(
      'Declared',
      [
        'pnpm --filter @aaes-os/aaes-governance test',
        'npm run verify --prefix docs-site',
        'npm run verify --prefix nova-studio',
      ],
      ['Fresh verification is recorded in the repo scorecard and docs hub rather than the release bundle itself.'],
    ),
    lintStatus: createEvidenceSection(
      ...
      ['Real workspace lint remains an open gap.'],
    ),
    replayStatus: createEvidenceSection(
      'Observed',
      ['packages/runledger', 'packages/evidence-receipts', 'release/signature.json'],
      ['Replay is reconstructed through the governed ledger and the release signature.'],
    ),
    ...
    knownLimitations: [
      'Real workspace lint remains an open gap.',
      'Release verification covers the selected artifacts, not every scaffolded surface.',
    ],
    truthBoundary:
